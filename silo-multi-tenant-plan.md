...
`apps/clustertenant-manager/src/infra/auth/oidc.service.ts`). It never **writes** to Zitadel — so a
...
`apps/clustertenant-manager/src/core/zitadel/zitadel-client.ts` + `_BuildZitadelManagementClient()` →
