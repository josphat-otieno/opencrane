...
describe("AG-UI stream state", function _Suite()
{
	...
	{
		const record = _Record("event-20", { type: "CUSTOM", name: "opencrane.source_references", value: { eventType: "source.references", messageId: "message-1", citations: [{ id: "cite-1", label: "Project brief", snippet: "safe", storageUrl: "https://storage.invalid" }], artifacts: [{ id: "artifact-ref-1", label: "brief.pdf", accessState: "metadata_only", mediaType: "application/pdf", lease: "secret" }], memoryReferences: [{ id: "memory-1", factId: "fact-1", contentDigest: "sha256:abc", rawFact: "never" }] } });
