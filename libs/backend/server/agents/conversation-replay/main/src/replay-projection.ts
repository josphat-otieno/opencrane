...
/** Redact one canonical row into only the fields the AG-UI projection contract allows. */
export function __ProjectConversationReplayEvent(row: ConversationReplayEventRow): ConversationReplayProjectionResult
{
	if (!row.cursor || !row.threadId || !row.runId || !Number.isSafeInteger(row.sequence) || row.sequence < 1 || !row.type || Number.isNaN(Date.parse(row.occurredAt))) return null;
	return { cursor: row.cursor, threadId: row.threadId, runId: row.runId, sequence: row.sequence, eventType: row.type, occurredAt: row.occurredAt, payload: _SafePayload(row.type, row.payload) };
}
