...
`SkillBundle` Prisma model (`apps/clustertenant-operator/prisma/schema.prisma`): `id`, `name`,
...
([skill-catalog.ts](https://github.com/italanta/opencrane/blob/main/apps/clustertenant-operator/src/routes/skill-catalog.ts)) and via
`oc skills …`.
...
2. **Scan.** `POST /api/v1/skills/catalog/:id/scan` runs a vulnerability scan
   ([scan-bundle.ts](https://github.com/italanta/opencrane/blob/main/apps/clustertenant-operator/src/core/scanning/scan-bundle.ts)): probes
...
4. The control plane's **`GET /api/internal/bundles/:digest/content?tenantName=…`**
   ([skill-bundles.ts](https://github.com/italanta/opencrane/blob/main/apps/clustertenant-operator/src/routes/internal/skill-bundles.ts))
