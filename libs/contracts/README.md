...
- `AG_UI_PROJECTION_VERSION`, `AgUiProjectionSourceEvent`, `AgUiSourceReferencesEvent`,
  `AgUiCitationReference`, `AgUiArtifactReference`, `AgUiMemoryReference`, `AgUiSseRecord`,
  `__ProjectAgUiEvent`, and `__EncodeAgUiSseRecord` — the display-safe input, output, source
  metadata, projection, and SSE-record contract used by the server-owned AG-UI replay path. They do
  not authenticate a browser, read canonical event storage, expose artifact bytes, or create an
