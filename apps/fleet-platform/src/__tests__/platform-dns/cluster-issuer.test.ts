...
import { _RenderDns01Issuer, _RenderDnsCredentialsSecret } from "../../core/platform-dns/cluster-issuer.js";
...
/** Narrow a rendered issuer to the acme fields under test. */
...
describe("_RenderDns01Issuer (CONN.8a)", function _suite()
{
	it("wires cloudflare apiTokenSecretRef to the credentials Secret", function _cf()
	{
		const acme = _acme(_RenderDns01Issuer(_config({ apiToken: "x" }), "opencrane-dns01-cloudflare"));
	...
	{
		const acme = _acme(_RenderDns01Issuer(_config({ provider: "digitalocean", apiToken: "x" }), "opencrane-dns01-digitalocean"));
	...
	{
		const acme = _acme(_RenderDns01Issuer(_config({ provider: "rfc2136", solverConfig: { nameserver: "10.0.0.1:53" } }), null));
		expect(acme.solvers[0].dns01.rfc2136).toEqual({ nameserver: "10.0.0.1:53" });
	});

	it("honours a custom ACME server", function _server()
	{
		const acme = _acme(_RenderDns01Issuer(_config({ apiToken: "x", server: "https://acme-staging.example/dir" }), "s"));
		expect(acme.server).toBe("https://acme-staging.example/dir");
	});

	it("throws when a token provider has no token", function _noTok()
	{
		expect(function _call() { _RenderDns01Issuer(_config(), null); }).toThrow(/requires an apiToken/);
	});

	it("throws when a non-token provider has no solverConfig", function _noCfg()
	{
		expect(function _call() { _RenderDns01Issuer(_config({ provider: "route53" }), null); }).toThrow(/requires a solverConfig/);
	});

	it("defaults to a cluster-wide ClusterIssuer with no namespace (legacy)", function _clusterDefault()
	{
		const issuer = _RenderDns01Issuer(_config({ apiToken: "x" }), "opencrane-dns01-cloudflare");
		expect(issuer.kind).toBe("ClusterIssuer");
		expect((issuer.metadata as { namespace?: string }).namespace).toBeUndefined();
	});

	it("renders a namespaced Issuer in the instance namespace when issuerKind=Issuer (MI.4)", function _namespaced()
	{
		const issuer = _RenderDns01Issuer(_config({ apiToken: "x", issuerKind: "Issuer", issuerNamespace: "oc-acme" }), "opencrane-dns01-cloudflare");
		expect(issuer.kind).toBe("Issuer");
		expect((issuer.metadata as { namespace?: string }).namespace).toBe("oc-acme");
	});
});
