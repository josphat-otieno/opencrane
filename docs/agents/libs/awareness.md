...
The awareness **contract-version module** used by the opencrane-api. Org-context RETRIEVAL is no
...
(`2-config-map.ts` `memory.contractVersion`) and drives the opencrane-api's awareness rollout/canary
...
[`feat-central-agents`](../apps/feat-central-agents.md) ingests org sources → Cognee. The opencrane-api
