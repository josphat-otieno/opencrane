 ...
 *
 * Run via:  pnpm --filter @opencrane/fleet-manager emit-openapi
 *
 * CI drift gate:
 *   pnpm --filter @opencrane/fleet-manager emit-openapi
 *   git diff --exit-code apps/fleet-manager/openapi.json
 *
...
// tsx resolves .js → .ts, so this import works both in tsx (dev) and after tsc.
