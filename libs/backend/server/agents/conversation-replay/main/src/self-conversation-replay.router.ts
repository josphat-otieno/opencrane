...
/** Create the browser-session-authenticated replay surface for a caller's own conversation. */
export function __CreateSelfConversationReplayRouter(dependencies: SelfConversationReplayRouterDependencies): Router
{
	const router = Router();
	router.get("/:threadId/events", async function _events(request: Request, response: Response)
	{
		...
		if (typeof threadId !== "string" || !threadId.trim() || cursor === undefined) { response.status(400).json({ error: "invalid_conversation_replay_request" }); return; }
		try
		{
			const events = await __ReadConversationReplay(dependencies.repository, { threadId, siloId: caller.siloId, subjectId: caller.subjectId, cursor, limit: 200 });
