...
/** Read one bounded, redacted canonical conversation snapshot through an already-authorised port. */
export async function __ReadConversationReplay(repository: ConversationReplayUnitOfWork, command: ReadConversationReplayCommand): Promise<readonly AgUiProjectionSourceEvent[]>
{
	if (!command.conversationId || !command.siloId || !command.subjectId || !Number.isSafeInteger(command.limit) || command.limit < 1 || command.limit > 500) return [];
