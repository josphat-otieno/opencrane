 ...
 *
 * The spec is the single source of truth (`apps/clustertenant-operator/openapi.json`,
...
const source = resolve(here, '../../apps/clustertenant-operator/openapi.json')
const dest = resolve(here, '../public/openapi.json')

if (!existsSync(source)) {
  console.error(
    `[sync-openapi] spec not found at ${source}\n` +
      `Run \`pnpm --filter @opencrane/clustertenant-operator emit-openapi\` first.`,
  )
  process.exit(1)
}
