...
describe("_ReviewToken", () =>
{
  ...
  it("rejects when authenticated but audience is missing skill-registry", async () =>
  {
    ...
    expect(result).toEqual({ ok: false, reason: "token audience does not include skill-registry" });
  });
