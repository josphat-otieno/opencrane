...
| Issue | Scope | Dependency |
|-------|-------|------------|
| [#150](https://github.com/italanta/opencrane/issues/150) — **Fleet↔silo contract + licensing split** | ClusterTenant CR schema + lifecycle API (incl. teardown, see #138) · OIDC delegation payload · relicense `fleet-operator`/`fleet-platform` to private ahead of the move | **Pull forward** — pure design + a licensing decision, moves no code. Start during the launch push; formally gates the rest of Phase E. |
| [#151](https://github.com/italanta/opencrane/issues/151) — **Standalone-capable silo** | Decouple identity (OIDC delegation), move `OrgDomainProvisioner` into the silo operator, chart self-sufficiency (SecretStore, NetworkPolicy floor, CRDs), standalone deploy mode | After #150. De-risks the fleet extraction; standalone value on its own. |
