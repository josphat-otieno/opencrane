...
/** Maximum time (ms) to wait on the opencrane-ui before giving up and falling back. */
const _FETCH_TIMEOUT_MS = 2000;

/**
 * Fetch the tenant's allowed model set from the opencrane-ui internal API.
 *
 * This is a deliberate, best-effort operator → opencrane-ui dependency: it MUST never
 ...
 *
 * @param controlPlaneUrl - In-cluster opencrane-ui base URL (no trailing slash needed).
...
{
  // 1. Missing config — without a opencrane-ui URL there is nothing to call. This is a
  ...
  {
    log.debug({ tenant }, "opencrane-ui URL unset; cannot resolve tenant-models");
    return { status: "error", modelSet: null };
  }

  // 2. Bounded request — abort after a short timeout so a hung opencrane-ui never
