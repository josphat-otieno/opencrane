...
- Phase 5 shipped (headless API + CLI). `oc` CLI, OpenAPI, generated `libs/contracts` client, and removal of `apps/clustertenant-manager-ui` in place.
...
| **Prisma migrations present** | ✅ Complete | `apps/clustertenant-manager/prisma/migrations/0001_init` committed |
...
3. **UI decoupled** — `apps/clustertenant-manager-ui` removed from this repo. Helm chart and installers no longer reference it. Platform operable via API + CLI only.
...
- `HostingAdapter` interface + `OnPremHostingAdapter` + `GcpHostingAdapter` in `apps/fleet-manager/src/hosting/`.
...
- `apps/clustertenant-manager-ui` removed from `pnpm-workspace.yaml` and deleted from repo.
...
- [x] `apps/clustertenant-manager-ui` removed; Helm chart/installers no longer reference it.
...
- [x] **P4A.2 Runtime-plane drift repair (operator config-slaving).** Added `RuntimePlaneDriftRepairer`
  (`apps/fleet-manager/src/runtime-planes/drift-repairer.ts`) — 60s interval compares Obot MCP gateway and
...
- [x] **MI.1 `multiInstance` Helm scaffold + namespaced operator RBAC (B1) + fail-closed watch (B2). — LANDED 2026-06-14.**
  ...
  `operator-deployment.yaml`, `apps/fleet-manager/src/config.ts`, `values.yaml`.
...
- [x] **MI.5 Per-component scope declaration + eliminate hard-coded names (B5). — LANDED 2026-06-14.** New `sharedPlatform` block (litellm/skillRegistry/mcpGateway/externalSecrets, `instance` default | `shared` opt-in, fail-fast guards); release-prefixed the non-templated `opencrane-obot` Secret + a second collision it caught (plain `litellm` Service); `config.ts` `opencrane-system` defaults now derive from `POD_NAMESPACE`. Build + 68 operator tests (+2) green; helm instance+shared modes. Original scope: Add a
  ...
  **shared** (`litellm`/`skillRegistry`/`mcpGateway`/`externalSecrets`), with the isolation
  ...
  `opencrane-system` service-URL defaults in `apps/fleet-manager/src/config.ts` (make them
  ...
  `external-secrets.yaml`, `apps/fleet-manager/src/config.ts`. **Headless-buildable.**
...
13. **Control-Plane Admin Surface (API + CLI)**
    ...
    - UI parity (if desired) is an external-consumer concern; `apps/clustertenant-manager-ui` was removed from this repo in Phase 5.
...
- [x] Projected-token migration: `aud=obot-gateway` and `aud=skill-registry` implemented in `apps/fleet-manager/src/tenants/deploy/3-deployment.ts`.
- [x] Real grant compilation: `apps/clustertenant-manager/src/core/grants/grant-compiler.ts` (scope precedence: priority → deny-over-allow → newest). `GET /tenants/:name/effective-contract` compiles Awareness/McpServer/SkillBundle grants. The `mcp.servers`/`skills.entitled` fields in `2-config-map.ts` are **intentionally advisory stubs** — authoritative grant is the effective-contract endpoint.
...
- [ ] **CONN.8 TLS issuance for tenant ingress (wildcard, k8s-native).** *First slice landed
  ...
  (`apps/fleet-manager/src/tenants/deploy/5-ingress.ts`) has **no `tls:` block** and Helm has
