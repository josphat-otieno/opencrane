...
3. **The selection is written into the effective-contract** as the per-skill (or per-tenant×skill) model recommendation. The pod re-pulls the contract at each agentic-loop boundary (as it already does for skill/MCP grants per [3-deployment.ts](apps/fleet-platform/src/tenants/deploy/3-deployment.ts)), so model choice propagates the same way grants do — no new propagation path.
