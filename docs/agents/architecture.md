...
- **`___AuthMiddleware` does NOT enforce per-route roles today** (`apps/clustertenant-platform/src/infra/middleware/auth.middleware.ts`). It's a fallback chain: public paths → OIDC cookie → env token → DB access token → dev bypass. Role/capability claims are a *planned* target — do not assume RBAC at the route layer.
