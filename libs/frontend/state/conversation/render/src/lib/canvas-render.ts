...
import { asFiniteNumber, asOptionalRecord } from "./shims/coerce";
...
function getRecordStringField(record: Record<string, unknown> | undefined, key: string): string | undefined
{
	const value = record?.[key];
	return typeof value === "string" && value.trim() ? value : undefined;
}

function getRecordNumberField(record: Record<string, unknown> | undefined, key: string): number | undefined
{
	const value = record?.[key];
...
function normalizeSurface(value: string | undefined): "assistant_message" | undefined
{
...
function coerceCanvasPreview(record: Record<string, unknown> | undefined): CanvasPreview | undefined
{
	if (!record)
	{
		return undefined;
...
function previewFromShortcode(attrs: Record<string, string>): CanvasPreview | undefined
{
	...
	const preferredHeight =
		attrs["height"] && Number.isFinite(Number(attrs["height"])) ? normalizePreferredHeight(Number(attrs["height"])) : undefined;
	...
	if (url || ref)
	{
		return {
			...
			surface,
			render: "url",
			url: url ?? defaultCanvasEntryUrl(ref as string),
			...(ref ? { viewId: ref } : {}),
			...(title ? { title } : {}),
			...(preferredHeight ? { preferredHeight } : {}),
			...(className ? { className } : {}),
...
export function extractCanvasFromText(outputText: string | undefined, _toolName?: string): CanvasPreview | undefined
{
	const parsed = tryParseJsonRecord(outputText);
	return coerceCanvasPreview(parsed);
}

/** Extracts `[embed …]` shortcodes outside code fences and returns the stripped text. */
export function extractCanvasShortcodes(text: string | undefined): { text: string; previews: CanvasPreview[] }
{
	...
	const matches: Array<{ start: number; end: number; attrs: Record<string, string>; body?: string }> = [];
