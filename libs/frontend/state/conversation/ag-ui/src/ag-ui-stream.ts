...
import type { AgUiArtifactReference, AgUiCitationReference, AgUiMemoryReference, AgUiSourceReferencesView, AgUiStreamRecord, AgUiStreamState } from "./ag-ui-stream.types.js";
...
{
	return { cursor: null, seenCursors: new Set(), runId: null, messages: {}, tools: {}, customEvents: [], sourceReferences: [] };
}
...
export function __ReduceAgUiStream(state: AgUiStreamState, record: AgUiStreamRecord): AgUiStreamState
{
	...
	if (event.type === "CUSTOM" && event.name === "opencrane.source_references" && _IsSourceReferencesEvent(event.value as Record<string, unknown>)) return _SourceReferences(state, seenCursors, record.id, event.value as Record<string, unknown>, event.name);
...
{
	return event["eventType"] === "source.references" && (event["messageId"] === undefined || typeof event["messageId"] === "string") && Array.isArray(event["citations"]) && Array.isArray(event["artifacts"]) && Array.isArray(event["memoryReferences"]) && event["citations"].every(_IsCitation) && event["artifacts"].every(_IsArtifact) && event["memoryReferences"].every(_IsMemoryReference);
}

/** Store one display-safe source-reference set after dropping unsafe extra fields. */
function _SourceReferences(state: AgUiStreamState, seenCursors: ReadonlySet<string>, cursor: string, event: Record<string, unknown>, name: string): AgUiStreamState
{
	const sourceReferences: AgUiSourceReferencesView = {
		...(event["messageId"] === undefined ? {} : { messageId: event["messageId"] as string }),
		citations: (event["citations"] as AgUiCitationReference[]).map(_CitationReference),
		artifacts: (event["artifacts"] as AgUiArtifactReference[]).map(_ArtifactReference),
		memoryReferences: (event["memoryReferences"] as AgUiMemoryReference[]).map(_MemoryReference)
	};
	return { ...state, seenCursors, cursor, sourceReferences: [...state.sourceReferences, sourceReferences], customEvents: [...state.customEvents, name] };
}
