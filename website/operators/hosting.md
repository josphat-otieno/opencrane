...
```typescript
// apps/fleet-platform/src/hosting/hosting-adapter.types.ts
...
```typescript
// apps/fleet-platform/src/hosting/adapters/onprem/onprem-hosting.adapter.ts
import type * as k8s from "@kubernetes/client-node";
...
```typescript
// apps/fleet-platform/src/hosting/adapters/gcp/gcp-hosting.adapter.ts
import type * as k8s from "@kubernetes/client-node";
...
```typescript
// apps/fleet-platform/src/hosting/hosting-adapter.factory.ts
...
1. **`optionalDependencies`** — each cloud SDK is declared under `optionalDependencies` in `apps/fleet-platform/package.json`, never `dependencies`. A normal `pnpm install` fetches them for development and cloud images; an on-prem image built with `pnpm install --no-optional` omits them entirely and still runs.
...
```typescript
// apps/fleet-platform/src/hosting/adapters/gcp/gcp-bucket.client.ts
