...
/**
 * Build the Express router for the feat-skill-registry delivery service.
 *
 * Delivery contract:
 ...
 *     (Authorization: Bearer <token>) bound to audience "feat-skill-registry".
 *   - The opencrane-ui's internal /api/internal/bundles/:digest/content endpoint
 ...
 * @param controlPlaneUrl - Base URL of the opencrane-ui (in-cluster).
...
export function _BuildRouter(authApi: k8s.AuthenticationV1Api, controlPlaneUrl: string, log: Logger): Router
{
  ...
  router.get("/bundles/:digest", async function _getBundle(req, res)
  {
    ...
    // Delegate entitlement check and content retrieval to the opencrane-ui internal endpoint.
    ...
    catch (err)
    {
      ...
      log.error({ digest, tenantName, err: message }, "failed to reach opencrane-ui internal endpoint");
    ...
    if (!upstream.ok)
    {
      log.error({ digest, tenantName, status: upstream.status }, "unexpected response from opencrane-ui internal endpoint");
